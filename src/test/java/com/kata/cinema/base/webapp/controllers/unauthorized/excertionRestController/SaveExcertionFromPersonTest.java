...
public class SaveExcertionFromPersonTest {
    ...
    @Test
    @Sql(scripts = "/data/sql/controllers/excertionRestController/saveFromPerson/init.sql", executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
    @Sql(scripts = "/data/sql/controllers/excertionRestController/saveFromPerson/delete.sql", executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
    public void saveFromPersonTest() throws Exception {
        ExcertionRequestDto excertionRequestDto = new ExcertionRequestDto("excertion description");

        mockMvc.perform(post("/api/persons/{id}/excertions", 101)
        ...
        Assertions.assertThat(excertion.getPerson().getId()).isEqualTo(101L);
    }
}
